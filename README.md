...
The simulation was utilised to determine optimal track banking angles to minimise flange contact time (the duration wheels grind against the rail), improving safety and reducing wear.

## Key Features
- **Multi-Body Physics:** Simulates $n$ bogies connected by springs and dampers, modelling the complex interaction between carriages.
...
- **Live Visualization:** Real-time animation using `matplotlib` to visualise lateral deviations and train pathing.

## Results
Based on the simulation model, an optimisation study was conducted for a quarter-circle turn (Radius = 300m).
...
4.  **Slipping/Creep:** Modelling the velocity vectors of wheels when lateral oscillation occurs.
